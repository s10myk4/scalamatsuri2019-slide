...
また、ユースケースを実行する、またはユースケースから実行されるような外界とのやりとりを担うアダプターの実装方針を決めることも行います。
...
ドメインモデルからソースコードで表現してみましょう。これは武器の定義です。武器の名前や攻撃力、属性や装備するのに必要なレベルを持ちます。
...
武器の実体はここで簡単のため定数として用意しています。実際にはこういった情報は何らかのストレージに保存されるでしょう。
...
ユースケースそのものの説明の前に、ユースケースの実行結果を表す型を定義しました。
...
ドメインロジックは特定のドメインモデルに閉じ込めることで業務を遂行する存在に限らず、ドメインの一貫性を保つことができます。

一方、これまで話にでた正常系、異常系はソフトウェアの要件を実現するための知識です。
この知識をユースケースの中に閉じたものとします。

これらはそれぞれ役割に応じた責務を持つ、凝集度の高いコンポーネントを実現します。
...
実装の側面においてはアーキテクチャについてもここで考えておきましょう。

実際にアプリケーションが価値を提供するためにはユースケースとドメインモデルだけではなく、これらを実行する、または実行されるアダプタが必要だからです。
...
同時に、このプロダクトが求められる性能や信頼性も時とともに変化していく必要があるでしょう。
...
- 概念モデルに対して同じ解釈ができているか
    - コードレビューで要件やモデリングに対する指摘が発生することを極力抑えられる

Note:

ここまでをチームで行った結果として、実装者同士が概念モデルに対して同じ解釈ができているか確認できます。
